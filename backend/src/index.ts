...
app.post('/api/chat', async (req: Request, res: Response) => {
  ...
  try {
    // Configuración del contexto y prompt de sistema
  ...
  "Si tienes alguna consulta previa, también puedo orientarte por aquí.";
    ...
    // Mapear historial al formato esperado por el SDK de Gemini
    // El formato del historial esperado por el SDK es un arreglo de Content objects
    // donde cada objeto tiene role: 'user' o 'model', y parts: [{ text: '...' }]
    ...
    try {
      ...
      console.warn("Gemini 2.5-flash falló (posible 503/sobrecarga), intentando con gemini-1.5-flash...", primaryErr);
      response = await aiClient.models.generateContent({
...
getDb().then(() => {
  ...
  console.error('No se pudo inicializar la base de datos SQLite:', err);
});
